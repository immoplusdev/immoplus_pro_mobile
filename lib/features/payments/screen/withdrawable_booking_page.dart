...
class WithdrawableBookingPage extends StatefulWidget {
  const WithdrawableBookingPage({super.key});
  static String name = 'WITHDRAW_BOOKING_PAGE';
  @override
  State<WithdrawableBookingPage> createState() =>
      _WithdrawableBookingPageState();
}

class _WithdrawableBookingPageState extends State<WithdrawableBookingPage> {
    ...
    LogmentRepository.getReservationsOwner(
      ...
      where: {
        '_where': [
          ...
          '{"_field": "retraitProEffectue", "_op": "eq", "_val": "false"}',
        ],
      },
    ).then((value) {
          ...
          PagedSliverList<int, ReservationModel>(
            ...
            builderDelegate: PagedChildBuilderDelegate(
              ...
              itemBuilder: (context, item, index) => InkWell(
                onTap: () {
                  AppRouter.router.pushNamed(OperatorsSelectorPage.name,
                      extra: PaymentPageAdapter(
                          itemId: item.id,
                          collection: 'reservation',
                          amount: item.montantTotalReservation.toInt()));
                },
                child: BookingCard(
                  clickable: false,
